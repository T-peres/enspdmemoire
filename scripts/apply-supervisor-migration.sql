-- =====================================================
-- APPLICATION DE LA MIGRATION - Interface Encadreur
-- =====================================================
-- Ce script applique toutes les nouvelles tables et fonctionnalités
-- pour l'interface encadreur

-- Vérifier si les tables existent déjà
DO $$
BEGIN
  IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'supervisor_meetings') THEN
    RAISE NOTICE 'Les tables existent déjà. Migration ignorée.';
  ELSE
    RAISE NOTICE 'Création des nouvelles tables...';
  END IF;
END $$;
...
BEGIN
  ...
  LEFT JOIN thesis_topics t ON t.student_id = sa.student_id AND t.supervisor_id = p_supervisor_id
...
LEFT JOIN thesis_topics t ON t.student_id = p.id AND t.supervisor_id = sa.supervisor_id
...
-- Tables créées: supervisor_meetings, intermediate_evaluations, document_comments, messages, alerts
-- Fonctions créées: create_alert(), get_supervisor_stats()
-- Vue créée: supervisor_students_overview
