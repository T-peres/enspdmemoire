-- =====================================================
-- FONCTIONNALITÉS ENCADREUR - Tables supplémentaires
-- =====================================================
...
CREATE INDEX idx_supervisor_meetings_theme ON supervisor_meetings(theme_id);
CREATE INDEX idx_supervisor_meetings_student ON supervisor_meetings(student_id);
CREATE INDEX idx_supervisor_meetings_supervisor ON supervisor_meetings(supervisor_id);
CREATE INDEX idx_supervisor_meetings_date ON supervisor_meetings(meeting_date DESC);
CREATE INDEX idx_supervisor_meetings_status ON supervisor_meetings(status);
...
CREATE TABLE IF NOT EXISTS intermediate_evaluations (
  ...
  evaluation_type TEXT NOT NULL, -- 'monthly', 'chapter', 'milestone'
  ...
  overall_score DECIMAL(4,2), -- Note sur 20
  strengths TEXT,
  weaknesses TEXT,
...
CREATE INDEX idx_intermediate_evaluations_theme ON intermediate_evaluations(theme_id);
CREATE INDEX idx_intermediate_evaluations_student ON intermediate_evaluations(student_id);
CREATE INDEX idx_intermediate_evaluations_supervisor ON intermediate_evaluations(supervisor_id);
CREATE INDEX idx_intermediate_evaluations_date ON intermediate_evaluations(evaluation_date DESC);
...
CREATE TABLE IF NOT EXISTS document_comments (
  ...
  -- Type de commentaire
  comment_type TEXT DEFAULT 'general', -- 'general', 'correction', 'suggestion', 'plagiarism_alert'
...
CREATE INDEX idx_document_comments_author ON document_comments(author_id);
...
CREATE TABLE IF NOT EXISTS messages (
  ...
  -- Contexte
  related_entity_type TEXT, -- 'theme', 'document', 'meeting', etc.
  ...
  -- Pièces jointes
  attachments JSONB, -- [{name, path, size, mime_type}]
...
CREATE TABLE IF NOT EXISTS alerts (
  ...
  alert_type TEXT NOT NULL, -- 'missing_meeting', 'document_pending', 'deadline_approaching', 'fiche_incomplete'
...
-- Fonction pour obtenir les statistiques d'un encadreur
...
BEGIN
  ...
  LEFT JOIN themes t ON t.student_id = sa.student_id AND t.supervisor_id = p_supervisor_id
  LEFT JOIN documents d ON d.theme_id = t.id
  LEFT JOIN supervisor_meetings sm ON sm.supervisor_id = p_supervisor_id
  LEFT JOIN alerts a ON a.user_id = p_supervisor_id
...
-- Policies pour supervisor_meetings
...
-- Policies pour intermediate_evaluations
...
-- Policies pour document_comments
...
-- Vue: Étudiants avec leur progression pour un encadreur
...
LEFT JOIN themes t ON t.student_id = p.id AND t.supervisor_id = sa.supervisor_id
